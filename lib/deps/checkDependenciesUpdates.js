...
// private
	...
	// methods
		...
		/**
		* @param {Array} dependencies : packages to check
		* @param {Object} options : used options for checking
		* @returns {Promise} : operation result
		*/
		function _execute (dependencies, options) {
...
module.exports = function checkDependenciesUpdates (dependencies, options) {
	...
	// force separate classic & dev
	return _execute(dependencies.filter((d) => {
		return !d.dev;
	}), options).then((depsValid) => {

		return _execute(dependencies.filter((d) => {
			return d.dev;
		}), options).then((devDepsValid) => {

			return Promise.resolve(depsValid && devDepsValid);

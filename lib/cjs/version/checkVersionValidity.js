...
function checkVersionValidity(version, strict = true) {
    return (0, checkNonEmptyString_1.default)(version).then(() => {
        if (strict) {
            return REGEX.test(version) ? Promise.resolve("RUNNABLE") : Promise.reject(new Error("\"version\" parameter (\"" + version + "\") does not follow the allowed patterns (\"" + REGEX_LITTERAL + "\")"));
        }
